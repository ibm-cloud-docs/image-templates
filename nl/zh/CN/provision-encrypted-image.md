...
* 最后，可以选择通过 [Activity Tracker](/docs/services/cloud-activity-tracker?topic=cloud-activity-tracker-activity_tracker_ov#activity_tracker_ov) 来审计与加密虚拟服务器关联的事件。
...
2. 使用 {{site.data.keyword.keymanagementservicefull_notm}} 来创建和管理密钥。
      ...
      4. [设置 Key Protect API](/docs/services/key-protect?topic=key-protect-set-up-api#set-up-api)，以便您可打包打算用于加密 VHD 映像的数据加密密钥。
      5. 使用根密钥[包装数据加密密钥](/docs/services/key-protect/wrap-keys.html#wrap-keys)。将加密映像导入到 {{site.data.keyword.cloud_notm}} 控制台时，将需要与打包的数据加密密钥 (WDEK) 关联的密文。
3. 通过 IBM {{site.data.keyword.iamshort}} (IAM)，[授予访问权](/docs/iam?topic=iam-serviceauth#create-auth)，即在 **Cloud Block Storage**（源服务）和 **{{site.data.keyword.keymanagementservicelong_notm}}**（目标服务）之间进行访问的权限。从 {{site.data.keyword.cos_full_notm}} 导入加密映像的用户必须在 IAM 中为 {{site.data.keyword.keymanagementservicelong_notm}} [定义访问策略](/docs/iam?topic=iam-userroles#userroles)。
4. 在 IBM Cloud 控制台中，创建 {{site.data.keyword.cos_full_notm}} 实例并创建存储区以存储数据。有关更多信息，请参阅 [{{site.data.keyword.cos_full_notm}} 入门教程](/docs/services/cloud-object-storage?topic=cloud-object-storage-getting-started-tutorial)。
      1. 在供应 {{site.data.keyword.keymanagementserviceshort}} 服务的区域位置中创建 {{site.data.keyword.cos_full_notm}} 实例。
      2. 创建存储区时，**弹性**设置必须为_区域_。
      3. （可选）创建存储区时，可以[使用密钥加密存储区](/docs/services/cloud-object-storage/basics?topic=cloud-object-storage-sse-kp#sse-kp)。   
...
5. 在 {{site.data.keyword.cos_full_notm}} 中，浏览至存储区，然后单击**添加对象**以[上传](/docs/services/cloud-object-storage?topic=cloud-object-storage-upload-data#upload-data)加密映像。可以使用 Aspera 高速传输插件来上传大于 200 MB 的映像。
